...
// ==========================================================================
// Credits: http://paypal.github.io/accessible-html5-lesson-player/
// ==========================================================================
...
(function (api) {
    ...
    var defaults = {
        ...
        classes: {
            videoWrapper: 'player-lesson-wrapper',
            embedWrapper: 'player-lesson-embed',
            type: 'player-{0}',
            stopped: 'stopped',
            playing: 'playing',
            muted: 'muted',
    ...
    function _supportMime(player, mimeType) {
        ...
        // Only check lesson types for lesson players
        if (player.type == 'video') {
            // Check type
            switch (mimeType) {
            case 'lesson/webm':
                return !!(media.canPlayType && media.canPlayType('lesson/webm; codecs="vp8, vorbis"').replace(/no/, ''));
            case 'lesson/mp4':
                return !!(media.canPlayType && media.canPlayType('lesson/mp4; codecs="avc1.42E01E, mp4a.40.2"').replace(/no/, ''));
            case 'lesson/ogg':
                return !!(media.canPlayType && media.canPlayType('lesson/ogg; codecs="theora"').replace(/no/, ''));
            }
        }
    ...
    function Plyr(container) {
        ...
        function _seekManualCaptions(time) {
            // If it's not lesson, or we're using textTracks, bail.
        ...
        function _findElements() {
            try {
                ...
                // Restore native lesson controls
        ...
        function _setupMedia() {
            ...
            if (!player.media) {
                _log('No audio or lesson element found!', true);
                return false;
            }

            if (player.supported.full) {
                // Remove native lesson controls
                player.media.removeAttribute('controls');

                // Add type class
                _toggleClass(player.container, config.classes.type.replace('{0}', player.type), true);

                // If there's no autoplay attribute, assume the lesson is stopped and add state class
                ...
                if (player.type === 'video') {
                    ...
                    // Wrap the lesson in a container
            ...
            if (player.type == 'youtube') {
                _setupYouTube(player.media.getAttribute('data-lesson-id'));
            }
        ...
        function _parseSource(sources) {
            ...
            // .source([{ src: 'path/to/lesson.mp4', type: 'lesson/mp4' },{ src: 'path/to/lesson.webm', type: 'lesson/webm' }])
        ...
        function _destroy() {
            ...
            // If lesson, we need to remove some more
            if (player.type === 'video') {
                // Remove captions
                _remove(_getElement(config.selectors.captions));

                // Remove lesson wrapper
        ...
        function _init() {
            ...
            player.media = player.container.querySelectorAll('audio, lesson, div')[0];
